...
req = requests.get('https://www.iocl.com/Product_PreviousPrice/PetrolPreviousPriceDynamic.aspx')
content = req.content
...
chennai_prices = []
petrol_prices = []
months_mapping = {"January": "01", "February": "02", "March": "03", "April": "04", "May": "05", "June": "06", "July": "07", "August": "08", "September": "09", "October": "10", "November": "11", "December": "12"}
for row in rows:
    cells = row.find_all('td')
    date = months_mapping[cells[0].get_text().replace('\n',
    '').split(',')[0].split(' ')[0]]+'-'+cells[0].get_text().replace('\n',
    '').split(',')[0].split(' ')[1]+'-'+cells[0].get_text().replace('\n',
    '').split(',')[1].replace(' ', '')
    # print(date)
    ...
    # print(date)
    petrol_price = [date, delhi_price, kolkata_price, mumbai_price, chennai_price]
